...
- (instancetype)init {
  if ((self = [super initWithFrame:CGRectMake(0, 0, Size, Size)])) {
    ...
    //        [self.layer setBackgroundColor:[[UIColor colorWithWhite:0
    //        alpha:0.5] CGColor]];
...
//    [self.layer setCornerRadius:10];
    ...
    //        initWithFrame:CGRectInset(self.bounds, 20, 20)];
    self.imageView = [[UIImageView alloc]
...
+ (void)dismiss {
  ...
  [[[GiFHUD instance] overlay] removeFromSuperview];
...
+ (void)setGifWithImageName:(NSString *)imageName {
  ...
  [[[self instance] imageView]
      setImage:[UIImage
                   animatedImageWithAnimatedGIFURL:[[NSBundle mainBundle]
...
+ (void)setGifWithURL:(NSURL *)gifUrl {
  [[[self instance] imageView] stopAnimating];
  [[[self instance] imageView]
      setImage:[UIImage animatedImageWithAnimatedGIFURL:gifUrl]];
}
